...
const Map = ({
  ...
  useEffect(() => {
    ...
    class HomeControl {
      onAdd(map) {
        ...
        this._container.className =
          "mapboxgl-ctrl mapboxgl-ctrl-group custom-map-control";
        ...
        (image.style.color = "#404040"), (image.style.fontSize = "20px");
    ...
    class LayerControl {
      onAdd(map) {
        ...
        const button = document.createElement("button");
        button.title = "Map layer";

        button.onclick = () => {
          setLayersOpen(!layersOpenRef.current);
        };

        const image = document.createElement("img");
        image.src = "icons/layers.svg";
        (image.style.color = "#404040"), (image.style.marginTop = "3px");
        button.appendChild(image);
    ...
    mapRef.current.addControl(new LayerControl(), "top-left");
  ...
  useEffect(() => {
    if (!!mapLoaded && mapRef.current.getLayer("locations"))
      mapRef.current.removeLayer("locations");
    if (!!mapLoaded && mapRef.current.getSource("locations"))
    ...
    if (busLocations && !!mapLoaded) {
      ...
      busLocations.map((itm) => {
        geojson.features.push({
          ...
          geometry: {
            type: "Point",
            coordinates: [Number(itm.CS_LNG), Number(itm.CS_LAT)],
          },
          properties: {},
        });
      });
